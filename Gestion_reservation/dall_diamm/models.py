...
for key, values in data.items():
    ...
    prix = values["prix"]
    date_livraison = changedate(values["date_livraison"]) 
    # Exécuter une requête SQL pour insérer les données
    with connection.cursor() as cursor:
        cursor.execute(
            "INSERT INTO dall_diamm_voiture(nom_voit, prix, date_livraison) "
            "VALUES (%s, %s, %s)",
            [nom_voit, prix, date_livraison]
        )
    voit = voiture(nom_voit=nom_voit, prix=prix, date_livraison=date_livraison)
    voit.save()
...
for key, values in contenu.items():
    numero_chambre = values["num_chmb"]
    nom_chambre = values["nom_chmb"]
    prix_chambre = values["prix"]
    date_location = changedate(values["date_livraison"]) 
    disponibilite=values["disponible"]
    
    # Exécuter une requête SQL pour insérer les données
    with connection.cursor() as cursor:
        cursor.execute(
            "INSERT INTO dall_diamm_chambre(num_chmb,nom_chmb, prix_chmb, date_location,disponible) "
            "VALUES (%s, %s, %s, %s, %s)",
            [numero_chambre,nom_chambre, prix_chambre, date_livraison,disponibilite]
        )
    chmb = chambre(num_chmb=numero_chambre,nom_chmb=nom_chambre, prix_chmb=prix_chambre, date_location=date_livraison,disponible=disponibilite)
    chmb.save()
