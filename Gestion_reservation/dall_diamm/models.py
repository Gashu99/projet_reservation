...
class personne(models.Model):
    nom=models.CharField(max_length=100,default='',null=True)
    prenom=models.CharField(max_length=100,default='',null=True)
    email=models.EmailField(default='',null=True)
    Tel=models.CharField(max_length=50)
    mot_de_passe=models.CharField(max_length=20,default='',null=True)
...
with open('/home/fatou/Bureau/projet_reservation/Gestion_reservation/dall_diamm/data_voit.json') as fichier:
    contenu_voiture = json.load(fichier)  
#Exécuter une requête SQL pour insérer les donnée  
idV = 1  
with connection.cursor() as cursor:
    for key, values in contenu_voiture.items():
        marque=values["marque"]
        model =values["model"]
        types= values["fuel_type"]
        ...
        prix=values["prix"]
        query_check="SELECT * FROM dall_diamm_voiture WHERE idV= %s"
        cursor.execute(query_check,[idV])
        resultat = cursor.fetchall()
        idV += 1 
        if not resultat:
            query_insert="""  
                INSERT INTO dall_diamm_voiture( marque, model,types,capacite,transmission,image,annee,couleur,prix) 
                VALUES (%s, %s,%s, %s, %s,%s, %s, %s,%s)
                """
            cursor.execute(query_insert,[marque, model,types,capacite,transmission,image,annee,couleur, prix])
    


class reservationVoitures(models.Model):
...
class restaurant(models.Model):
    id_plat=models.IntegerField(primary_key=True)
    nom_plat=models.CharField(max_length=200)
    prix_plat=models.DecimalField(max_digits=8,decimal_places=2)
    img_plat=models.ImageField()

# with open('/home/fatou/Bureau/projet_reservation/Gestion_reservation/dall_diamm/data_resto.json') as fichier:
#     contenu_resto = json.load(fichier)

#     with connection.cursor() as cursor:
#         for values in contenu_resto:
#             id_plat = values["id_plat"]
#             nom_plat = values["nom_plat"]
#             prix_plat = values["prix_plat"] 
#             img_plat = values["id_plat"] 
#             query_check1="SELECT * FROM dall_diamm_restaurant WHERE id_plat= %s"
#             cursor.execute(query_check1,[id_plat])
#             resultat1 = cursor.fetchall()

#             if not resultat1:
#                 query_insert1="""  
#                     INSERT INTO dall_diamm_restaurant(id_plat, nom_plat, prix_plat, img_plat) 
#                     VALUES (%s, %s, %s, %s)
#                     """
#                 cursor.execute(query_insert1,[id_plat, nom_plat, prix_plat, img_plat])